...
class ZjuMocapReader():
    ...
    def read_captures(cls, scene_dir, tgt_size, EVERY_K):
        ...
        # make up for selected views
        human_id = os.path.basename(scene_dir)
        if human_id=='d16': 
            slc_ids = [204, 832, 844]
        if human_id=='d17': 
            slc_ids = [564, 1228, 1252]
        if human_id=='d18': 
            slc_ids = [204, 708, 736]
        if human_id=='d19': 
            slc_ids = [212, 408, 556]
        if human_id=='d20': 
            slc_ids = [204, 700, 712]
        slc_ids = (np.array(slc_ids)-200)//4
        ...
        if EVERY_K==75:
            ...
            human_id = os.path.basename(scene_dir)
            if human_id=='d16': 
                slc_ids = [200, 564, 824, 832, 840]
            if human_id=='d17': 
                slc_ids = [200, 564, 1228, 1240, 1252]
            if human_id=='d18': 
                slc_ids = [200, 500, 708, 736, 780]
            if human_id=='d19': 
                slc_ids = [212, 280, 408, 556, 564]
            if human_id=='d20': 
                slc_ids = [200, 240, 644, 700, 712]
            slc_ids = (np.array(slc_ids)-200)//4
    ...
    def read_smpls(cls, scene_dir, EVERY_K):
        ...
        slc_smpl_paths = []
        for i in range(3):
            slc_smpl_paths.append(smpl_paths[slc_ids[i]])
        ...
        for i, path in enumerate(slc_smpl_paths):
            temp_smpl = (
                np.load(path, allow_pickle=True).item()
            )
            temp_smpl['pose'] = temp_smpl.pop('poses')[0]
