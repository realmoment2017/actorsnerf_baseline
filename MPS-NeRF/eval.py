...
def test(chunk, render_kwargs, savedir=None, global_args=None):
    ...
    for i, data in enumerate(test_loader):
        ...
        for k in range(0, tp_input['rgb_all'].shape[1]):
            ...
            for j in range(batch_size):
                ...
                filename = os.path.join(os.path.join(savedir, os.path.basename(global_args.ft_path)[:-4]),
                                '{}_{}_frame_{}.png'.format(global_args.finetune_subject[-3:], k+1,
                                os.path.basename(test_set.ims[tp_input['pose_index']][k])[-10:-4]))
...
if __name__=='__main__':
    ...
    render_kwargs_train, render_kwargs_test, start, grad_vars, _ = create_nerf(global_args)
    testsavedir = os.path.join(global_args.basedir, global_args.expname)
    test(global_args.chunk, render_kwargs_test, savedir=testsavedir, global_args=global_args)
