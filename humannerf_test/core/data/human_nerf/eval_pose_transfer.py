...
##                cam_idxs = [3,15]
        ...
        self.use_AIST_pose = False
        if self.use_AIST_pose:
            self.target_sub_idx = 'd03'
            dataset_path = f"datasets/AIST_mocap/{self.target_sub_idx}"
        else:
            self.target_sub_idx = '396'
            dataset_path = f"tools/prepare_zju_mocap/dataset/zju_mocap/{self.target_sub_idx}"
...
#                    first_camera = False
    ...
    def get_freeview_camera(self, target_camera, frame_idx, total_frames, trans=None):
        E = rotate_camera_by_frame_idx(
                extrinsics=target_camera['extrinsics'],
                frame_idx=frame_idx,
                period=total_frames,
                trans=trans,
                **self.ROT_CAM_PARAMS[self.target_src_type[self.target_sub_idx]])
        K = target_camera['intrinsics'].copy()
...
#        return K, E
    ...
    def __getitem__(self, idx):
        ...
        # sub depends on the recon human id
        # cam depends on the target / recon human
        # idx depends on the target poses
        sub_idx = self.idx_hist[0]['sub_idx']
        cam_idx = self.idx_hist[0]['cam_idx']
        frame_idx = 'frame_000000'
        target_sub_idx = self.target_sub_idx
        target_cam_idx = self.target_idx_hist[idx]['cam_idx']
        target_frame_idx = self.target_idx_hist[idx]['frame_idx']


        bgcolor = np.array(self.target_bgcolor[target_sub_idx], dtype='float32')

        ########### target human ###############
        img, _ = self.load_image(sub_idx, cam_idx, frame_idx, bgcolor)
        # img, _ = self.load_image_target(target_sub_idx, target_cam_idx, target_frame_idx, bgcolor)
        ...
        # dst_skel_info = self.query_dst_skeleton_target(target_sub_idx, target_cam_idx, 'frame_000970')
...
#            dst_Rh = dst_skel_info['Rh']
#            dst_Th = dst_skel_info['Th']
#            dst_bbox = {'min_xyz': np.array([-0.73424781, -1.40589068, -0.32033295]), 
#                        'max_xyz': np.array([0.71842316, 0.64778442, 0.60724758])}
#            dst_Rh = np.array([-1.4717202, -0.5415103,  0.607263 ]) 
#            dst_Th = np.array([ 0.4370668 ,  0.35106644, -1.114761  ])
        ########### overwrite tpose joints with recon human ###############
        dst_tpose_joints = self.train_mesh_info[sub_idx][cam_idx][frame_idx]['tpose_joints'].astype('float32') # update which trainset frame

        ########### recon human cam / target human cam ##################
        use_recon_human_cam = True
        if use_recon_human_cam:
            target_camera = self.cameras[sub_idx][cam_idx][frame_idx] # update with recon dataset
            K = target_camera['intrinsics'][:3, :3].copy()
            K[:2] *= cfg.resize_img_scale
            E = target_camera['extrinsics']
            results = {
                'sub_idx': sub_idx,
                'cam_idx': cam_idx,
